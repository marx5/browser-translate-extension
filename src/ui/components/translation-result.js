...
class TranslationResult extends BaseComponent {
  ...
  displayResult(result) {
    ...
    if (result.targetPhonetic) {
      const phonetic = DOMUtils.createElement(
        "div",
        "inline-phonetic collapsed"
      );
      phonetic.id = "targetPhoneticContainer";
      phonetic.style.display = "flex";

      const phoneticText = DOMUtils.createElement(
        "span",
        "inline-phonetic-text"
      );
      phoneticText.textContent = `/${result.targetPhonetic}/`;
      phonetic.appendChild(phoneticText);

      // Toggle button - default collapsed
      const toggleBtn = DOMUtils.createElement(
        "button",
        "inline-phonetic-toggle"
      );
