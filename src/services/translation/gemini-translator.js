...
class GeminiTranslator extends BaseTranslator {
  ...
  constructor(phoneticService, apiUrl, apiKey) {
    ...
    this.apiKey = apiKey || "";
    this.model = "gemini-2.5-flash-lite"; // Changed from gemini-2.5-flash-lite
  }
  ...
  handleProxyError(proxyResponse) {
    ...
    if (
      ...
      return new Error(
        "Gemini API: API key không hợp lệ hoặc chưa được cấu hình. Vui lòng kiểm tra API key trong Tab Settings."
      );
    }
    ...
    if (
      ...
      return new Error(
        "Gemini API key không hợp lệ. Vui lòng kiểm tra lại key trong Tab Settings."
      );
    }
    ...
    if (
      ...
      return new Error(
        "Gemini API: Đã vượt quá giới hạn số lượt gọi. Vui lòng thử lại sau."
      );
    }

    if (status === 400) {
      return new Error(
        `Gemini API lỗi: ${errorMessage || "Yêu cầu không hợp lệ"}`
      );
    } else if (status === 429) {
      return new Error(
        "Gemini API: Đã vượt quá giới hạn số lượt gọi. Vui lòng thử lại sau."
      ...
      return new Error(
        "Gemini API: Không có quyền truy cập. Vui lòng kiểm tra API key."
      );
    } else if (status === 401) {
      return new Error(
        "Gemini API: API key không được xác thực. Vui lòng kiểm tra lại."
      );
    } else if (status >= 500) {
...
PHONETICS:
• For Japanese: Use romaji, group by word (e.g., "arigatou gozaimasu")
• For Chinese: Use pinyin with tones, GROUP SYLLABLES BY WORD (e.g., "Jīqíng sì shè" NOT "Jī qíng sì shè"). 
  - Multi-syllable words should be written together: "niánqīng" not "nián qīng", "gānjìng" not "gān jìng"
  - Only separate distinct words with spaces
• For Korean: Use romanization, group by word (e.g., "annyeonghaseyo")
